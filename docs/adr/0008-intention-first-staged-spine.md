...
first-class — but they are now _sequenced and scope-gated_, not two full lenses drawn up front. The
...
an agent guess-to-confirm. The agent _traces the real current codebase_ from the locked goal to
...
the developer can correct is a _trace that missed a caller or dependency_ — that fixes the analysis,
...
drawn, so a misread of intent costs _one_ target redraw, never a downstream cascade.

## Principles this rests on

- **The diagram is the message (diagram-first).** Loupe relieves cognitive load by letting the
  developer decide by _looking_. Falling back to prose to convey the model defeats the purpose. Text
...
- **Forks only.** A question earns a place only if (a) answering it differently changes the spec/plan
  _materially_ and (b) it genuinely needs the developer's judgment (the agent cannot resolve it from
  code/context). Everything the agent _can_ infer becomes a **confirmable assumption shown on the
...
- **Visual option-compare.** A fork's options are compared as small outcome _diagrams_ side by side
...
- **Acceptance Criteria** — the conditions that must hold for the change to be accepted, spanning
  **functional** requirements (what it must do) _and_ **non-functional** ones (how well — latency,
  ...
  the spec _complete_, not just transcribe it. A proposed criterion the developer didn't mean also
...
- **before → after** — one diagram, one purpose (the single relationship the change turns over),
  with a one-line takeaway. Says _where we're going_.
- **work-DAG** — the execution plan as a graph: nodes are concrete changes, edges are
  ...
  where they'll be seen _before_ approval. Says _in what order, and what could bite_. It collapses to a
...
- **validation pass** — the agent must _prove with the design_ that it satisfies the ① acceptance
...
- **detail refinement** (the overall goal still holds) → _diff-aware re-derivation_: walk the ⟵
...
- **deviation from the goal itself** → _from scratch_: the goal is the root of the derivation tree,
